...
object RoutingTable {
  ...
  : Iterator[RoutingMessage] = {
    ...
    val mirrorSize = graphPartition.mirrorSize
    val lDMirrorSPos = graphPartition.smallDegreeEndPos
    val lDMirrorEPos = graphPartition.largeDegreeMirrorEndPos
    val lDMasterEPos = graphPartition.largeDegreeMasterEndPos
    val lDMirrorSize = lDMirrorEPos - lDMirrorSPos
    // println(mirrorSize + " " + lDMirrorSPos + " " + lDMirrorEPos + " " + lDMasterEPos)

    val outMirrorSPos = graphPartition.largeDegreeMasterEndPos
    val map = Array.fill(mirrorSize)(0x0.toByte)
    val local2global = graphPartition.local2global

    // start from smallDegreeEndPos
    if (needSrc) {
      graphPartition.remoteLDMirrorIterator.foreach { v =>
        val index = v - lDMirrorSPos
        // println("Remote In Mirrors: " + v)
        // val prev = map(index)
        map(index) = 0x1.toByte
      }
    }
    ...
    if (needDst) {
      ...
      graphPartition.remoteOutMirrorIterator.foreach { v =>
        val index = if (v < lDMirrorEPos) v - lDMirrorSPos
        else v - lDMasterEPos + lDMirrorSize
    ...
    map.zipWithIndex.filter(_._1 != 0x0.toByte).map { v =>
      val localVid = if (v._2 < lDMirrorSize) v._2 + lDMirrorSPos
      else v._2 - lDMirrorSize  + lDMasterEPos
  ...
  */
  ...
  RoutingTable = {
    ...
    for (msg <- iter) {
      ...
      // println("Routing Message: " + pid + " " + localVid)
